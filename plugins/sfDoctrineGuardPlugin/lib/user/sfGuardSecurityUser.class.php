...
class sfGuardSecurityUser extends sfBasicSecurityUser
{
  ...
  /**
   * Método propio (Creado) donde se obtiene el objeto SAF_UNIDAD_EQUIPO al que
   * pertenece el usuario SF_GUARD_USER
   * 
   * @return type
   */
  public function getUnidadEquipo()
  {
    return Doctrine_Core::getTable('SAF_UNIDAD_EQUIPO')->find($this->getGuardUser()->getIdUe());
  }
  
  /**
   * Método propio (Creado) que retorna un boolean dependiendo si la unidad
   * perteneciente al usuario SF_GUARD_USER tiene el compromiso pasado como parametro.
   * 
   * @param int $id_compromiso
   * @return boolean
   */
  public function hasCompromiso($id_compromiso)
  {
    $compromiso = Doctrine_Core::getTable('SAF_COMP_UE')->findCompromisoDeLaUnidad($this->getGuardUser()->getIdUe(),$id_compromiso);    
